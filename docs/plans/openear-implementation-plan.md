...
| 3 | Bot layer (auth, handlers, keyboards, formatters) | Group 1 + Group 2 | Yes -- auth, keyboards, formatters are independent; handlers depends on those 3 |
| 4 | Scheduler (jobs, integration) | Group 2 + Group 3 | No -- single step |
| 5 | Main entry point, end-to-end wiring | Group 1-4 | No -- single step |
| 6 | Scripts (Gmail OAuth, deploy) | Group 1 | Yes -- both scripts are independent |
...
Group 3 ──┬── 3A (auth.py)
          ├── 3B (keyboards.py)
          ├── 3C (formatters.py)
          └── 3D (handlers.py) ← depends on 3A, 3B, 3C
               │
...
**Verify**: `python -c "from src.db.models import Base, Email, Reminder, Note; print('Tables:', [t for t in Base.metadata.tables])"` prints all six table names.
...
**Time**: ~4 min
**What**: Creates the SQLite engine, provides an async-compatible session factory, and handles table creation. Uses `INSERT OR IGNORE` pattern via a helper for email dedup.
...
class ReminderService:
    ...
    def create_reminder(
        ...
        with get_session() as session:
            ...
            logger.info("Created reminder #%d: %s at %s", reminder_id, title, due_at)
    ...
    def snooze_reminder(
        ...
        Duration format: "1h", "30m", "tomorrow", "1d".
...
class NoteService:
    ...
    def save_note(self, content: str, tags: list[str] | None = None) -> Note:
        ...
        with get_session() as session:
            ...
            session.flush()
            note_id = note.id
            logger.info("Saved note #%d with tags %s", note_id, tags or [])
...
class HealthService:
    ...
    def get_memory_usage(self) -> str:
        ...
        try:
            ...
            # getrusage returns KB on macOS, pages on Linux
            rusage = resource.getrusage(resource.RUSAGE_SELF)
            mem_mb = rusage.ru_maxrss / 1024  # macOS reports in bytes
            if os.uname().sysname == "Linux":
                mem_mb = rusage.ru_maxrss / 1024  # Linux reports in KB
    ...
    def get_last_backup_info(self) -> str:
        ...
        try:
            with get_session() as session:
                ...
                if result:
                    return result.timestamp.strftime("%Y-%m-%d %H:%M UTC")
                return "Never"
        except Exception:
            return "Unknown"
...
Steps 3A, 3B, and 3C have no dependencies on each other (only on Group 1 + 2) and can be implemented in parallel. Step 3D depends on all three.
...
def format_briefing(emails: list[dict], tz_name: str = "America/Los_Angeles") -> str:
    ...
    Args:
        emails: list of dicts with keys: sender, subject, summary, label,
                received_at
...
### Step 3D: Bot handlers (`src/bot/handlers.py`)

**File**: `src/bot/handlers.py`
**Time**: ~5 min
**What**: All Telegram command and message handlers. Depends on auth, keyboards, formatters, and all services. Manages conversation context with LLM summarization.
...
- Keeps the last 20 messages as active context
...
class BotHandlers:
    ...
    async def cmd_briefing(
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                "Please re-authenticate via /reauth or scripts/setup_gmail.py."
            )
    ...
    # ---- Message Handler (general conversation + intent routing) ----
    ...
    async def handle_message(
        ...
        if intent == "weather":
            ...
            self._store_conversation("user", user_message)
            self._store_conversation("assistant", response)

            # Check if summarization is needed
            await self._maybe_summarize_context(update)
    ...
    def _store_conversation(self, role: str, content: str) -> None:
        """Store a conversation message in the database."""
        with get_session() as session:
            session.add(Conversation(role=role, content=content))

    async def _get_conversation_context(self) -> list[dict[str, str]]:
        ...
        with get_session() as session:
            ...
            # Get recent messages (excluding summaries)
            recent = (
                session.execute(
                    select(Conversation)
    ...
    async def _maybe_summarize_context(self, update: Update) -> None:
        ...
        if count and count > MAX_ACTIVE_MESSAGES:
            ...
            with get_session() as session:
                ...
                existing = (
                    session.execute(
                        select(ConvModel)
                        .where(ConvModel.role == "context_summary")
                ...
                if existing:
                    ...
                    session.add(
                        ConvModel(role="context_summary", content=summary)
                    )
    ...
    async def _email_briefing_job(self) -> None:
        ...
        try:
            ...
            await self._send_to_all(
                ...
                "Re-authenticate via scripts/setup_gmail.py."
            )
        except Exception as e:
...
Steps 6A, 6B, and 6C are independent of each other and can be implemented in parallel. They depend only on Group 1 (config).
...
**File**: `.env.example` (update existing -- clarify non-sensitive only)

```
# === openEar Configuration ===
# This file contains NON-SENSITIVE config only.
# Secrets (API keys, tokens) come from AWS SSM Parameter Store in production.
# For local development, you can set secrets here.
...
# 3. In Telegram, send these messages to your bot:
...
| `src/bot/handlers.py` | 3 | 3D | Create |
...
**Total steps**: 20 (across 8 groups)
**Estimated time**: 75-90 minutes of focused implementation
**Maximum parallelism**: Group 1 (3 parallel), Group 2 (7 parallel), Group 3 (3 parallel + 1 dependent), Group 6 (3 parallel)
