...
class BotHandlers:
    ...
    def _track_chat(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        logger.info("track_chat: chat_id=%s type=%s", chat_id, update.effective_chat.type)
    ...
    async def cmd_whitelist(
        ...
        if sub_cmd.startswith("add "):
            ...
            with get_session() as s:
                existing = s.query(SenderWhitelist).filter_by(pattern=email, chat_id=chat_id).first()
                if existing:
                    await update.message.reply_text(f"'{email}' is already in the whitelist as '{existing.label}' 🐰")
                else:
                    s.add(SenderWhitelist(pattern=email, label=label, chat_id=chat_id))
                    await update.message.reply_text(f"✅ Added '{email}' as '{label}' 🐰")

        elif sub_cmd.startswith("remove "):
            email = sub_cmd[7:].strip()
            with get_session() as s:
                entry = s.query(SenderWhitelist).filter_by(pattern=email, chat_id=chat_id).first()
                if entry:
                    s.delete(entry)
            ...
            with get_session() as s:
                entries = s.query(SenderWhitelist).filter_by(chat_id=chat_id).all()
                for e in entries:
                    s.expunge(e)
            if not entries:
    ...
    async def _process_single_intent(
        ...
        if intent == "weather":
            ...
            email = intent_data.get("email", "").strip().lower()
