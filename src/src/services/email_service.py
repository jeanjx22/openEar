...
class EmailService:
    ...
    def has_whitelist_entries(self, chat_id: int) -> bool:
        """Check if a chat has any whitelist entries."""
        with get_session() as session:
            return session.query(SenderWhitelist).filter_by(chat_id=chat_id).count() > 0

    def _match_whitelist(self, sender: str, chat_id: int | None = None) -> str | None:
        ...
        If chat_id is provided, only checks that chat's whitelist.
        """
        with get_session() as session:
            query = session.query(SenderWhitelist)
    ...
    async def process_emails(self, chat_id: int | None = None) -> list[dict]:
        """Fetch, classify, summarize, and store emails.

        Returns a list of important email dicts ready for briefing.
        If chat_id is provided, whitelist matching is scoped to that chat.
        """
        ...
        for i, email in enumerate(raw_emails):
            label = self._match_whitelist(email["sender"], chat_id=chat_id)
            if label:
                email["label"] = label
